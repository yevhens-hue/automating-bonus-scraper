...
export default function BookmakerTable({ bookmakers, teamHome, teamAway }: BookmakerTableProps) {
    ...
    const maxOdds = useMemo(() => {
        if (!bookmakers || bookmakers.length === 0) {
            return { max1: 0, maxX: 0, max2: 0 };
        }
        return {
            max1: Math.max(...bookmakers.map(b => b.odds["1"] || 0)),
            maxX: Math.max(...bookmakers.map(b => b.odds["X"] || 0)),
            max2: Math.max(...bookmakers.map(b => b.odds["2"] || 0))
        };
    }, [bookmakers]);
                ...
                <table className="w-full text-left text-sm text-gray-300">
                    ...
                    <tbody>
                        {sortedAndFiltered.map((bm, index) => {
                            ...
                            const isMax1 = bm.odds["1"] === maxOdds.max1 && maxOdds.max1 > 0;
                            const isMaxX = hasDraw && bm.odds["X"] === maxOdds.maxX && maxOdds.maxX > 0;
                            const isMax2 = bm.odds["2"] === maxOdds.max2 && maxOdds.max2 > 0;
